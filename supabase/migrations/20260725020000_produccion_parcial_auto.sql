-- ============================================================
-- MIGRACIÓN: Reposición automática + cancelación de órdenes
--
-- Contexto: en vez de exigir un clic manual para generar la orden de
-- reposición del faltante, se genera automáticamente en el momento en
-- que una orden de producción queda definitivamente 'parcial' (es decir,
-- ya no le quedan ítems en 'planificada'/'en_proceso' — todos están
-- completada/parcial/cancelada). Como contrapeso, se agrega
-- fn_cancelar_orden_produccion para poder cancelar esa orden de
-- reposición (u otra orden sin producción registrada) si no se va a
-- fabricar el faltante.
-- ============================================================

-- ------------------------------------------------------------
-- 1. fn_completar_item_produccion: dispara la reposición automática
--    cuando la orden queda 'parcial' y ya no hay ítems pendientes.
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 2. fn_cancelar_orden_produccion: cancela una orden (y sus ítems)
--    siempre que no tenga producción registrada. Sirve tanto para
--    órdenes normales sin empezar como para reposiciones automáticas
--    que ya no se van a fabricar.
-- ------------------------------------------------------------
