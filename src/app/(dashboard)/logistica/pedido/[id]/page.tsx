...
export default function PedidoCockpitPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function handleCambiarEstado(nuevoEstado: EstadoPedido, bolsas?: number) {
    ...
    try {
      ...
      if (nuevoEstado === "confirmado" && pedido.estado === "fecha_tentativa") {
        ...
        const updates: Database["public"]["Tables"]["pedidos"]["Update"] = { estado: nuevoEstado };
  ...
  return (
    <div className="max-w-[1400px] mx-auto space-y-4">
      ...
      {loading ? (
        ...
        <div className="flex flex-col md:flex-row gap-6 border rounded-lg bg-white">
          {/* LEFT PANEL */}
          <div className="flex-1 px-4 sm:px-6 py-4 space-y-4">
            ...
            <section className="space-y-2">
                ...
                <div className="flex items-start gap-1.5">
                  ...
                  <span>
                    {pedido.metodo_pago
                      ...
                      : "—"}
                    {" · "}
                    <span className={pedido.estado_pago === "confirmado" ? "text-success" : "text-warning"}>
              ...
              {pedido.notas_ventas && (
                <div className="mt-2 p-2.5 rounded-md bg-warning/10 border border-warning/20 text-sm">
                  ...
                  <p className="text-warning whitespace-pre-wrap">{pedido.notas_ventas}</p>
                </div>
              )}

              {/* Dispatch notes */}
              {pedido.notas_despacho && (
                <div className="mt-1 p-2.5 rounded-md bg-info/10 border border-info/20 text-sm">
                  <p className="text-xs font-semibold text-info mb-1">Notas de despacho</p>
                  <p className="text-info whitespace-pre-wrap">{pedido.notas_despacho}</p>
                </div>
              )}
            </section>
                  ...
                  <div key={nota.id} className="flex items-start gap-2">
                    ...
                    >
                      {nota.completada ? (
                        ...
                        <Circle className="h-4 w-4 text-muted-foreground hover:text-success transition-colors" />
                      )}
                    </button>
          ...
          <div className="w-full md:w-80 lg:w-96 md:shrink-0 md:border-l border-t md:border-t-0 flex flex-col">
            <div className="px-4 py-3 border-b">
              <p className="text-xs font-semibold uppercase text-muted-foreground tracking-wide flex items-center gap-1">
                <History className="h-3.5 w-3.5" /> Historial
              </p>
            </div>
            <div className="flex-1 overflow-y-auto px-3 sm:px-4 py-3 space-y-3 max-h-[600px]">
              {actividad.length === 0 && (
